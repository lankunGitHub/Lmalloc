...
// 自适应调整 cache_bin 容量
// ind：bin索引，收缩时需把超出的缓存对象归还对应arena bin
static void cache_bin_adapt(cache_bin_t* bin, szind_t ind)
{
    // 每个bin独立节流（曾用函数级static共享所有线程/bin的节流窗口，
    // 且非原子读改写存在数据竞争）
    uint64_t now = tcache_now();
    if (now - bin->adapt_epoch < TCACHE_BIN_ADAPT_INTERVAL)
        return;
    bin->adapt_epoch = now;
    // 容量上限取全局可调值（lmalloc_auto_tune会调整它）
    size_t cap_max = tcache_bin_max_global > TCACHE_BIN_MAX ? TCACHE_BIN_MAX
                                                            : tcache_bin_max_global;
    if (cap_max < TCACHE_BIN_MIN)
        cap_max = TCACHE_BIN_MIN;
    double hit_rate =
        (double)bin->hit_count / (bin->hit_count + bin->miss_count + 1e-9);
    if (hit_rate > TCACHE_HIT_THRESHOLD && bin->capacity < cap_max)
    {
        bin->capacity *= 2;
        if (bin->capacity > cap_max)
            bin->capacity = cap_max;
    }
    else if (hit_rate < TCACHE_MISS_THRESHOLD && bin->capacity > TCACHE_BIN_MIN)
    {
        ...
        // 收缩时超出新容量的缓存对象归还arena：
        // 曾直接截断丢弃，region位图永远保持已用造成永久泄漏
        if (bin->head > bin->capacity)
        {
            arena_t* arena = arena_choose(tsd_fetch());
            while (bin->head > bin->capacity)
            {
                void* ptr = bin->stack[--bin->head];
                arena_dalloc_small(arena, ptr, ind);
            }
        }
    }
...
static void cache_bin_gc_lru(cache_bin_t* bin, arena_t* arena, bin_t* ab)
{
    ...
    for (int i = 0; i < nflush && bin->head > 0; ++i)
    {
        ...
        // 经arena_dalloc_small回收（内部加arena锁，
        // 曾直接调bin_dalloc_region无锁并发操作bin链表）
        arena_dalloc_small(arena, bin->head ? bin->stack[lru_idx] : NULL,
                           (unsigned)(ab - arena->bins));
        // 移除该对象
...
{
    (void)size;
    if (unlikely(ind >= sz_nbins || ind >= TCACHE_NBINS_MAX))
        return NULL;
    tcache_t* tcache = tsd_tcachep_get(tsd);
    cache_bin_t* bin = &tcache->bins[ind];
    cache_bin_adapt(bin, ind);
    ...
    if (batch < 1)
        batch = 1;
    // 栈上临时数组：曾用base_alloc且永不释放，每次refill泄漏约batch*8字节
    void* batch_arr[TCACHE_BIN_MAX / 2];
    int got = 0;
...
{
    // binind必须合法：process_handoff会直接用它索引tcache->bins，
    // 越界（负值或>=TCACHE_NBINS_MAX）会导致野写
    if (binind < 0 || (size_t)binind >= TCACHE_NBINS_MAX)
    {
        arena_t* arena = arena_choose(tsd_fetch());
        arena_dalloc_small(arena, ptr, (szind_t)binind);
        return;
    }
...
static void tcache_process_handoff(tcache_t* tcache)
{
    for (int i = 0; i < tcache_handoff_head; ++i)
    {
        ...
        if (bin->head < bin->capacity)
        {
            // 直接入栈也要维护LRU时间戳（曾漏写，GC会把刚
            // handoff进来的对象当成最久未用而优先淘汰）
...
void tcache_flush_all_bins(tcache_t* tcache)
{
    if (!tcache)
        return;
    // 先处理handoff队列，使其条目随本轮回收到arena
    // （曾放在flush之后：对象被压回刚清空的bin，再也不会归还arena）
...
{
    // 未初始化线程（从未分配过内存）的thread_tcache为NULL
    if (!thread_tcache)
        return;
